...
음성 파일 업로드 → STT 변환, 감정 인식(SER), care 감정 분류, mood-light 색상 push까지 한 번에 처리. 세션 turn에도 자동 기록됨.
...
세션의 대화 맥락(turns)을 바탕으로 AI 응답 생성. 생성된 응답은 세션에 assistant turn으로 자동 저장.
...
| `emotions` | dict[string, float] | Y | 감정별 확률/점수 |
...
세션 종료. 지금까지의 감정 평균 계산, 수면 컬러 프로필 산출, mood-light에 sleep 색상 push.
...
- `emotions`: SER 모델이 산출한 raw 감정 확률 분포 (예: `happy`, `sad`, `angry`, `neutral` 등).
- `care_emotion` / `dominant_emotion`: 서비스 로직(케어 목적)으로 재분류/집계된 감정 라벨.
