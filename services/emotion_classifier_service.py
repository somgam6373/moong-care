...
CLASSIFIER_SYSTEM_PROMPT = (
    ...
    "emotion2vec 점수는 1순위 하나만 보지 말고 1순위와 2순위를 같이 봐. 예를 들어 "
    "surprised가 1순위여도 2순위가 happy면 '기쁜 놀람/설렘'에 가깝고, 2순위가 "
    "fearful/sad/disgusted처럼 부정적이면 놀람 속에 불안이나 불쾌가 섞여 있다고 봐야 해. "
    ...
    "우선 고려해.\n"
    "2) 반대로 텍스트가 칭찬/감사/인정하는 긍정적인 표현인데, emotion2vec 점수에서 "
    "disgusted, angry, sad 중 하나가 1순위 또는 2순위이면서 그 값이 무시 못 할 크기면(대략 0.3 이상), "
    ...
    "중 하나만 골라서 써야 해.\n"
    "이 규칙은 disgusted/angry/sad 중 하나가 1순위나 2순위에 실제로 있고 그 점수가 "
    "대략 0.3 이상일 때만 적용해. 1·2순위가 surprised/happy/fearful/neutral 조합처럼 "
    "disgusted/angry/sad를 포함하지 않으면 이 규칙은 적용하지 말고, 긍정적인 텍스트를 "
    "그대로 존중해서 joy/calm/relief/excitement 같은 감정을 고려해 — 텍스트가 우연히 "
    "예전에 봤던 예시와 비슷하다고 무조건 의심하지 마, 반드시 그 turn의 실제 점수로 판단해.\n"
