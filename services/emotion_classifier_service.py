...
def _fallback(reason: str) -> CareEmotionResult:
...
def _parse_response(content: str) -> CareEmotionResult:
    try:
        data = json.loads(content)
    except (TypeError, json.JSONDecodeError):
        return _fallback("classifier returned invalid json")

    care_emotion = data.get("care_emotion")
    if care_emotion not in ALLOWED_CARE_EMOTIONS:
        return _fallback("classifier returned unsupported care_emotion")

    try:
        confidence = float(data.get("confidence", 0.0))
    except (TypeError, ValueError):
        return _fallback("classifier returned invalid confidence")
