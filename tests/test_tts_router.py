...
def test_synthesize_returns_wav_bytes_with_default_voice(monkeypatch):
    ...
    def fake_synthesize_stream(text, voice, instructions):
    ...
    monkeypatch.setattr(tts_router.tts_service, "synthesize_stream", fake_synthesize_stream)
...
def test_synthesize_uses_requested_voice(monkeypatch):
    ...
    monkeypatch.setattr(tts_router.tts_service, "synthesize_stream", lambda text, voice, instructions: iter([b"RIFF"]))
...
def test_synthesize_rejects_unsupported_voice(monkeypatch):
    ...
    def _boom(text, voice, instructions):
        raise AssertionError("synthesize_stream should not be called for invalid voice")

    monkeypatch.setattr(tts_router.tts_service, "synthesize_stream", _boom)
